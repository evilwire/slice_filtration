...
For $X$ a $k$-scheme, let $\codim{1}{X}$ denote the collection of 
...
Furthermore, for $x, y \in X$, define
\[
\ptres{x}{y}: M(x) \to M(y)
\]
by
...
\ptres{x}{y} = 
\begin{cases}
0 & y\textbf{ is not a codim one subscheme in the closed 
    subscheme defined by }x \\
\sum_{z|y} 
\end{cases}
\]
